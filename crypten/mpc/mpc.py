...
class MPCTensor(CrypTensor):
    ...
    def pairwise_max(self, y):
        from .gw_relu_helper import gw_get_msb,mixed_mul_aby3
        #print('run my softmax')
        sign_bit = gw_get_msb((self-y)._tensor)
        if comm.get().get_rank() == 0:
            sign_bit ^= 1
        pairwise_max = mixed_mul_aby3((self-y)._tensor,  sign_bit) + y
        return pairwise_max
